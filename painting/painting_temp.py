...
class Painter:
    def __init__(self, seg_net_index):
        ...
        if seg_net_index == 0:
            ...
            self.model = init_model(config_file, checkpoint_file,
                                    device='cuda:0')  # TODO edit here if you want to use different device
    ...
    def get_score(self, idx, left):
        ...
        if self.seg_net_index == 0:
            ...
            # class에 맞게 softmax값의 max값으로 재구성
    ...
    def augment_lidar_class_scores_both(self, class_scores_r, class_scores_l, lidar_raw, projection_mats):
        ...
        lidar_raw : (115384, 4)
        class_scores_r,class_scores_l : (370, 1224, 5)
        """
        ...
        ################################
        lidar_cam_coords = self.cam_to_lidar(lidar_raw, projection_mats)  # (115384, 4)

        # right
        # array([0.  , 0.  , 0.  , ..., 0.34, 0.41, 0.  ]) -> array([1., 1., 1., ..., 1., 1., 1.])
        lidar_cam_coords[:, -1] = 1  # homogenous coords for projection, 마지막 열
        # TODO: change projection_mats['P2'] and projection_mats['R0_rect'] to be?
        points_projected_on_mask_r = projection_mats['P3'].dot(
            projection_mats['R0_rect'].dot(lidar_cam_coords.transpose()))  # (4, 115384)
        points_projected_on_mask_r = points_projected_on_mask_r.transpose()  # (115384, 4)
        points_projected_on_mask_r = points_projected_on_mask_r / (
            points_projected_on_mask_r[:, 2].reshape(-1, 1))  # (115384,)-> (115384,1)
        ...
        true_where_point_on_img_r = true_where_x_on_img_r & true_where_y_on_img_r  # bool array

        pdb.set_trace()
        points_projected_on_mask_r = points_projected_on_mask_r[  # (33191, 4)
            true_where_point_on_img_r]  # filter out points that don't project to image
        points_projected_on_mask_r = np.floor(points_projected_on_mask_r).astype(  # 정수 변환
       ...
       [547, 364]])
        ...
        augmented_lidar = np.concatenate((lidar_raw, np.zeros((lidar_raw.shape[0], class_scores_r.shape[2]))),
                                         axis=1)  # (115384, 9)
        ...
        augmented_lidar = augmented_lidar[true_where_point_on_img]  # (33896, 9)
